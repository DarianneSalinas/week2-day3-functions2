//afternoon-projects week2-day3 -functions2
...
//   //first for loop will loop through the array
//   for (let i =0; i < arr.length; i++) {
//     //second for loop will check if the values are duplicate by comparing them to i?
...
//  const each = (arr, callback) => 
//    arr.forEach((element, i) => callback(elem, i))
...
//invoking the each function by passing names array and callback function
...
// const getUserById = (userArr, id, callback) => {
...
const getUserById = (user, id, cb) => {
  //users refers to entire array
  //forEach allows us to look at each inidvidual object, user
  users.forEach(user => {
    if (user.id === id) {
      cb(user)
    }
